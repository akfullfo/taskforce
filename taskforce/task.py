...
"""
    ...
    def handle(self, details=None):
        ...
        log.debug("Received %s(%s) event for %r, details: %s",
                self._handler_name, '' if self._handler_arg is None else self._handler_arg, self._name, details)
    ...
    """
        ...
        if status:
            ...
            log.info("pid %d for %s(%s) %s", pid, self._name, self._handler_arg, why)
    ...
    """
        ...
        if isinstance(ret, list):
            if len(ret) == 0:
                ...
                raise TaskError(name, "Value %r resolved to a multi-valued list %s" % (value, ret))
        else:
            raise TaskError(name, "Value %r resolved to unexpect type %s" % (value, type(ret).__name__))
    ...
    def _sig_handler(self, sig, frame):
        ...
        if sig == signal.SIGCHLD:
            ...
            try:
                ...
                log.error("Write to self-pipe failed -- %s", e)
            return
        ...
        if sig in self._signal_prior and type(self._signal_prior[sig]) == type(self._sig_handler):
            log.info("Chaining to prior signal handler %s", self._signal_prior[sig])
            try:
                self._signal_prior[sig](sig, frame)
            except Exception as e:
                log.error("Exception from chained handler %s -- %s", self._signal_prior[sig], e)
            else:
                log.info("Prior signal handler %s returned", self._signal_prior[sig])
    ...
    """
        ...
        if self._config_file:
            try:
                ...
                log.error("Load of config from %r failed -- %s", self._config_file, e)
                return False
        else:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if e.errno != errno.EINTR:
                        ...
                        log.debug("Ignoring %s(%s) during poll", e.__class__.__name__, e)
                ...
                if idle_starving or not evlist:
                    ...
                    if self._reload_config:
                        try:
                            ...
                            log.error("Config load sequence failed -- %s", e, exc_info=True)
                    ...
                    for item, mask in evlist:
                        ...
                        for tgt in item.get():
                            if isinstance(tgt, tuple):
                                ...
                                path = tgt
                                if path not in self._file_event_map:
                                    log.error("Ignoring unknown file path %r from event", path)
                                    continue
            ...
            if not self._exiting:
                ...
                try:
                    ...
                    log.error("Failsafe attempt to stop tasks failed -- %s", ee)
            for server in self._http_servers:
                if server:
                    try: self._pset.unregister(server)
                    except: pass
    ...
    def close(self):
        ...
        if self._legion:
            try:
                ...
                log.warning("Task %r event deregister failed -- %s", self._name, e, exc_info=log.isEnabledFor(logging.INFO))
            try:
                self._legion.task_del(self)
            except Exception as e:
                log.warning("Task %r legion delete -- %s", self._name, e, exc_info=log.isEnabledFor(logging.INFO))
        else:
    ...
    def _make_event_target(self, event, control):
        ...
        for h in ['command', 'signal']:
            ...
            if val:
                ...
                break
        if not handler:
            raise TaskError(self._name, "Event type %r has no handler defined" % event.get('type'))
